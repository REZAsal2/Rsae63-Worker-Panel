...
// @ts-ignore
// https://github.com/bia-pain-bache/Reza-Worker-Panel
...
// https://www.nslookup.io/domains/Reza.yousef.isegaro.com/dns-records/
const proxyIPs= ['Reza.yousef.isegaro.com'];
const defaultHttpPorts = ['80', '8080', '2052', '2082', '2086', '2095', '8880'];
const defaultHttpsPorts = ['443', '8443', '2053', '2083', '2087', '2096'];
let proxyIP = proxyIPs[Math.floor(Math.random() * proxyIPs.length)];
let dohURL = 'https://cloudflare-dns.com/dns-query';
let trojanPassword = `Reza-trojan`;
            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {
                ...
                switch (url.pathname) {
                    ...
                    case '/panel':
                        ...
                        if (typeof env.Reza !== 'object') {
                            const errorPage = renderErrorPage('KV Dataset is not properly set!', null, true);
                            return new Response(errorPage, { status: 200, headers: {'Content-Type': 'text/html'}});
                        }

                        const pwd = await env.Reza.get('pwd');
                        ...
                        const proxySettings = await env.Reza.get('proxySettings', {type: 'json'});
                    ...
                    case '/login':
                        ...
                        let secretKey = await env.Reza.get('secretKey');
                        if (!secretKey) {
                            secretKey = generateSecretKey();
                            await env.Reza.put('secretKey', secretKey);
                        }

                        if (request.method === 'POST') {
                            const password = await request.text();
                            const savedPass = await env.Reza.get('pwd');
                    ...
                    case '/panel/password':
                        ...
                        const oldPwd = await env.Reza.get('pwd');
...
async function updateDataset (env, Settings, resetSettings) {
    ...
    if (!resetSettings) {
        try {
            currentProxySettings = await env.Reza.get("proxySettings", {type: 'json'});
        } catch (error) {
            console.log(error);
            throw new Error(`An error occurred while getting current values - ${error}`);
        }
    } else {
        await env.Reza.delete('warpConfigs');
    }
    ...
    try {    
        await env.Reza.put("proxySettings", JSON.stringify(proxySettings));          
    } catch (error) {
        console.log(error);
...
async function Authenticate (request, env) {
    ...
    try {
        const secretKey = await env.Reza.get('secretKey');
...
async function renderHomePage (env, hostName, fragConfigs) {
    ...
    try {
        proxySettings = await env.Reza.get('proxySettings', {type: 'json'});
        warpConfigs = await env.Reza.get('warpConfigs', {type: 'json'});
        password = await env.Reza.get('pwd');
    } catch (error) {
        console.log(error);
	...
	<head>
        ...
        <title>Reza Panel ${panelVersion}</title>
	...
	<body>
		<h1>Reza Panel <span style="font-size: smaller;">${panelVersion}</span> ®️👹</h1>
		<div class="form-container">
			...
			<div class="table-container">
				<table id="normal-configs-table">
					...
					<tr>
						...
						<td>
                            <button onclick="openQR('https://${hostName}/sub/${userID}#Reza-Normal', 'Normal Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}#Reza-Normal', false)">
					...
					<tr>
						...
						<td>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}?app=singbox#Reza-Normal', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
						</td>
					</tr>
                    <tr>
                        ...
                        <td>
                            <button onclick="openQR('sing-box://import-remote-profile?url=https://${hostName}/sub/${userID}?app=sfa#Reza-Normal', 'Normal Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}?app=sfa#Reza-Normal', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <button onclick="openQR('https://${hostName}/sub/${userID}?app=clash#Reza-Normal', 'Normal Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}?app=clash#Reza-Normal', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
                    </tr>
			...
			<div class="table-container">
                <table id="frag-sub-table">
                    ...
                    <tr>
                        ...
                        <td>
                            <button onclick="openQR('https://${hostName}/fragsub/${userID}#Reza Fragment', 'Fragment Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/fragsub/${userID}#Reza Fragment', true)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <button onclick="openQR('https://${hostName}/fragsub/${userID}?app=hiddify#Reza-Fragment', 'Fragment Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/fragsub/${userID}?app=hiddify#Reza-Fragment', true)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
                    </tr>
			...
			<div class="table-container">
				<table id="normal-configs-table">
					...
					<tr>
						...
						<td>
                            <button onclick="openQR('https://${hostName}/warpsub/${userID}?app=xray#Reza-Warp', 'Warp Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/warpsub/${userID}?app=xray#Reza-Warp', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
					</tr>
					<tr>
						...
						<td>
                            <button onclick="openQR('sing-box://import-remote-profile?url=https://${hostName}/warpsub/${userID}?app=singbox#Reza-Warp', 'Warp Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/warpsub/${userID}?app=singbox#Reza-Warp', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
						</td>
					</tr>
                    <tr>
                        ...
                        <td>
                            <button onclick="openQR('https://${hostName}/warpsub/${userID}?app=clash#Reza-WARP', 'Warp Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/warpsub/${userID}?app=clash#Reza-WARP', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
                    </tr>
			...
			<div class="table-container">
				<table id="warp-pro-configs-table">
					...
					<tr>
						...
						<td>
                            <button onclick="openQR('https://${hostName}/warpsub/${userID}?app=nikang#Reza-Warp-Pro', 'Warp Pro Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/warpsub/${userID}?app=nikang#Reza-Warp-Pro', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
					</tr>
					<tr>
						...
						<td>
                            <button onclick="openQR('sing-box://import-remote-profile?url=https://${hostName}/warpsub/${userID}?app=hiddify#Reza-Warp-Pro', 'Warp Pro Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/warpsub/${userID}?app=hiddify#Reza-Warp-Pro', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
						</td>
					</tr>
            ...
            <div class="footer">
                ...
                <a class="link" href="https://github.com/bia-pain-bache/Reza-Worker-Panel" target="_blank">Github</a>
    ...
    <body>
        <div id="error-container">
            ...
            <div id="error-message">
                <h2>${message} ${refer 
                    ? 'Please try again or refer to <a href="https://github.com/bia-pain-bache/Reza-Worker-Panel/blob/main/README.md">documents</a>' 
                    : ''}
                </h2>
...
async function fetchWgConfig (env, warpKeys) {
    ...
    await env.Reza.put('warpConfigs', JSON.stringify(warpConfigs));
}
...
async function buildWorkerLessConfig(remoteDNS, localDNS, lengthMin,  lengthMax,  intervalMin,  intervalMax, blockAds, bypassIran, blockPorn, bypassLAN, bypassChina, blockUDP443) {
    ...
    fragConfig.remarks  = '®️👹 Reza F - WorkerLess ⭐'
...
async function getFragmentConfigs(env, hostName) {
    ...
    if (outProxy) {
        ...
        try {
            ...
            await env.Reza.put("proxySettings", JSON.stringify({
                ...proxySettings, 
                outProxy: '',
                outProxyParams: ''
            }));
    ...
    bestPing.remarks = '®️👹 Reza F - Best Ping 🪄';
    ...
    bestFragment.remarks = '®️👹 Reza F - Best Fragment 😎';
...
async function getXrayWarpConfigs (env, client) {
    ...
    xrayWarpBestPing.remarks = client === 'nikang' ? '®️👹 Reza - Warp Pro Best Ping ✌️🧨' : '®️👹 Reza - Warp Best Ping ✌️🧨';
    ...
    xrayWarpOutbounds.forEach((outbound, index) => {
        xrayWarpConfigs.push({
            ...xrayWarpConfig,
            remarks: client === 'nikang' ? `®️👹 Reza - Warp Pro ${index + 1} 🇮🇷` : `®️👹 Reza - Warp ${index + 1} 🇮🇷`,
    ...
    xrayWoWOutbounds.forEach((outbound, index) => {
        if (outbound.tag.includes('warp-out')) {
            ...
            xrayWoWConfig.remarks = client === 'nikang' ? `®️👹 Reza - WoW Pro ${index/2 + 1} 🌍` : `®️👹 Reza - WoW ${index/2 + 1} 🌍`;
    ...
    xrayWoWBestPing.remarks = client === 'nikang' ? '®️👹 Reza - WoW Pro Best Ping ✌️🧨' : '®️👹 Reza - WoW Best Ping ✌️🧨';
...
async function getClashConfig (env, hostName, isWarp) {
    ...
    config['proxy-groups'][0].proxies = isWarp
        ? ['®️👹 Warp Best Ping ✌️🧨', '®️👹 WoW Best Ping ✌️🧨', ...warpOutboundsRemarks, ...wowOutboundRemarks ]
        : ['®️👹 Best Ping 🪄', ...outboundsRemarks ];

    config['proxy-groups'][1].proxies = isWarp ? warpOutboundsRemarks : outboundsRemarks;
    config['proxy-groups'][1].name = isWarp ? `®️👹 Warp Best Ping ✌️🧨`: `®️👹 Best Ping 🪄`,

    isWarp && config["proxy-groups"].push({
        "name": "®️👹 WoW Best Ping ✌️🧨",
        "type": "url-test",
