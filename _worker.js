            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {
                ...
                switch (url.pathname) {
                    ...
                    case '/panel':
                        ...
                        if (! await env.Reza.get('proxySettings')) {
                    ...
                    case '/login':
                        ...
                        let pwd = await env.Reza.get('pwd');


                        if (!pwd) {
                            await env.Reza.put('pwd', 'rsae');
                        }
...
const getNormalConfigs = async (env, hostName, client) => {
    let vlessWsTls = '';
    const { cleanIPs } = await env.Reza.get("proxySettings", {type: 'json'});
...
const buildWorkerLessConfig = async (env, client) => {
    let proxySettings = await env.Reza.get("proxySettings", {type: 'json'});
    ...
    fragConfig.remarks  = '👹Reza Frag - WorkerLess ⭐'
...
const getFragmentConfigs = async (env, hostName, client) => {
    ...
    bestPing.remarks = '👹Reza Frag - Best Ping 💥';
...
const getSingboxConfig = async (env, hostName) => {
    const {
        ...
        localDNS,
        cleanIPs
    } = await env.Reza.get("proxySettings", {type: 'json'});
...
const renderHomePage = async (env, hostName, fragConfigs) => {
	...
	<body>
		<h1>Reza Panel <span style="font-size: smaller;">${panelVersion}</span> 🌜</h1>
			...
			<div class="table-container">
				<table id="normal-configs-table">
					...
					<tr>
						...
						<td>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}#Reza%20Normal', false)">
                                Copy Sub
	...
	<script>
        ...
        const applySettings = async (event, configForm) => {
            ...
            try {
                ...
                if (response.ok) {
                    alert('Parameters applied successfully ✌️😎');
    ...
    <body>
        <div id="error-container">
            ...
            <div id="error-message">
                <h2>${message} Please try again or refer to <a href="https://github.com/bia-pain-bache/Reza-Worker-Panel/blob/main/README.md">documents</a></h2>
